        ...
        while (index < entrada.length) {
            ...
            if (estado === 0) {
                if (char === '/' && entrada[index + 1] === '/') {
                    ...
                    index += 2;
                    columna += 2;
                    while (index < entrada.length && entrada[index] !== '\n') {
                        buffer += entrada[index];
                        index++;
                        columna++;
                    } this.listaTokens.push(new Token("COMENTARIO", buffer.trim(), linea, columna));
                    continue;
                }
                if (char === "{") {
                    columna += 1;
...
export class Parser {
    ...
    forStatement() {
        ...
        // Parte 2: condición
        const condicion = this.expresion(); // i < 10
        this.avanzar(); // ;

        // Parte 3: incremento
        const incremento = this.expresion(); // i++
        this.avanzar(); // )
        this.coincidir("LLAVE_CIERRA");
    }
    bloque() {
...
/*class Traductor {
  ...
  traducir() {
    while (this.index < this.tokens.length) {
      ...
      // Traducir declaración con asignación: int x = 5;
      if (token.tipo === "INT" || token.tipo === "FLOAT") {
        const nombre = this.tokens[this.index + 1]?.lexema;
        const igual = this.tokens[this.index + 2]?.lexema;
        const valor = this.tokens[this.index + 3]?.lexema;

        if (nombre && igual === "=" && valor) {
          this.resultado += `${nombre} = ${valor}\n`;
          this.index += 5; // incluye el punto y coma
          continue;
        }
      }

      // Traducir asignación simple: x = 5;
      if (token.tipo === "IDENTIFICADOR" && this.tokens[this.index + 1]?.lexema === "=") {
        const nombre = token.lexema;
        const valor = this.tokens[this.index + 2]?.lexema;
        this.resultado += `${nombre} = ${valor}\n`;
        this.index += 4; // incluye el punto y coma
        continue;
      }
      if(token.tipo === "IF"){
        while (this.tokenActual()?.lexema !== "{") this.avanzar();
      ...
      this.avanzar(); // avanzar si no coincide con nada
    }
