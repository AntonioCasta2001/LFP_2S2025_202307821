...
export class AnalizadorLexico {
    ...
    esOperadorCompuesto(char) {
        const compuestos = ["==", "!=", "<=", ">=", "++", "--"];
        return compuestos.includes(char);
    }
    esOperadorSimple(char){
        const simples = ["+", "-", "*", "/", "=", "<", ">"];
        return simples.includes(char);
    }
    esSimbolo(char) {
        const simbolos = ["{", "}", "(", ")", "[", "]"]
    ...
    analizar(entrada) {
        ...
        while (index < entrada.length) {
            ...
            if (estado === 0) {
                ...
                } else if (this.esOperadorSimple(char) || this.esOperadorCompuesto(entrada.slice(index, index + 2))) {
